...
MAX=1000000
# 수집 창 기본 1일 — 매일 1회 돌리는 전제. 화면(통계) 창은 그대로 롤링 45일이다
# (update.sh 의 export --days 45). DB 에 쌓인 과거 매치는 남아 있으니 영향 없다.
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --seed|--seed-nicknames)
      [[ $# -ge 2 ]] || { echo "$1 값이 필요합니다." >&2; exit 1; }
      EXTRA_ARGS+=(--seed-nicknames "$2"); shift 2 ;;
    --max) MAX="$2"; shift 2 ;;
