...
기본은 **1일** — 매일 한 번 돌리는 전제다. 화면(통계) 창은 그대로 롤링 45일이다
(`update.sh` 의 `export --days 45`). DB 에 쌓인 과거 매치는 지워지지 않으니
수집 창을 좁혀도 리더보드 45일 집계는 그대로 나온다.
...
> **순위 추이가 수집 규칙성에 묶인다.** 페이지의 `변동` 뱃지와 추이 차트는 롤링 45일 창을
