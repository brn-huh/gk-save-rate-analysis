...
_SHOT_COLS = (
    "match_id, gk_ouid, gk_sp_id, gk_sp_grade, shot_type, result, "
...
def rebuild(con: duckdb.DuckDBPyConnection) -> ParseStats:
    ...
    for mid, payload in con.execute("SELECT match_id, payload FROM raw_match").fetchall():
        ...
        apps.extend((x.match_id, x.gk_ouid, x.gk_sp_id, x.gk_sp_grade) for x in a)
        shots.extend(
            (
                x.match_id, x.gk_ouid, x.gk_sp_id, x.gk_sp_grade, x.shot_type, x.result,
                x.is_pk, x.in_penalty, x.assist, x.hit_post, x.x, x.y,
            )
            for x in s
        )
    if apps:
        con.executemany("INSERT INTO gk_match VALUES (?, ?, ?, ?)", apps)
    if shots:
        con.executemany(
            f"INSERT INTO shot ({_SHOT_COLS}) VALUES ({', '.join(['?'] * 12)})", shots
        )
    return stats
...
def card_leaderboard(
    con: duckdb.DuckDBPyConnection, *, gate: int = MIN_MATCHES_GATE
) -> list[dict[str, Any]]:
...
def grade_breakdown(con: duckdb.DuckDBPyConnection, sp_id: int) -> list[dict[str, Any]]:
    """한 카드의 강화단계별 세부 (표본수 포함)."""
    rows = con.execute(
        """
        WITH m AS (
            SELECT gk_sp_grade, count(DISTINCT match_id) AS matches
            FROM gk_match WHERE gk_sp_id = ? GROUP BY gk_sp_grade
        ),
        s AS (
            SELECT gk_sp_grade,
                   sum(CASE WHEN result = 1 THEN 1 ELSE 0 END) AS saves,
                   sum(CASE WHEN result = 3 THEN 1 ELSE 0 END) AS goals
            FROM shot WHERE NOT is_pk AND gk_sp_id = ? GROUP BY gk_sp_grade
        )
...
def within_ouid_grade_effect(
    con: duckdb.DuckDBPyConnection, *, min_matches_per_cell: int = 5
) -> dict[str, Any]:
    ...
    rows = con.execute(
        ...
        WITH m AS (
            ...
            FROM gk_match GROUP BY 1, 2, 3
        ),
        s AS (
            SELECT gk_ouid, gk_sp_id, gk_sp_grade,
                   sum(CASE WHEN result = 1 THEN 1 ELSE 0 END) AS saves,
                   sum(CASE WHEN result = 3 THEN 1 ELSE 0 END) AS goals
            FROM shot WHERE NOT is_pk GROUP BY 1, 2, 3
        )
