...
보정하지 않은 종합선방률은 카드 성능이 아니라 그 카드를 쓰는 유저 실력이 섞인 값이므로
카드 순위가 아니라 참고용 지표임을 산출물에서 명시한다.
"""
...
WARNING = (
    ...
    "따라서 카드 순위가 아니라 참고용 지표로 볼 것. 강화 자체의 효과는 grade_effect(유저 내 비교)를 볼 것. "
...
def build_payload(
    ...
    *,
    ...
    # "최근 창 순위 − 전체 기간 순위" 를 빼는 꼴이 되어 델타가 거짓말을 한다.
