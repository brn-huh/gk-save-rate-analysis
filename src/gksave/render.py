...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .trendsvg{width:100%;height:auto;max-height:270px;background:#0e1424;
        border:1px solid var(--line);border-radius:9px}
  .tlab{fill:var(--mut);font-size:14px}
  /* 점 위에 마우스를 올리면 그 시점 값을 즉시 띄운다(기본 title 툴팁은 지연이 있다).
     paint-order:stroke 로 어두운 외곽선을 깔아 선·격자와 겹쳐도 읽힌다. */
...
<div class="panel help" id="panel-help">
  ...
  <ol class="usage">
    ...
    <li>맨 오른쪽 <b>변동</b> 컬럼은 <b>약 1주 전과 견준 선방률 순위</b>의 오르내림입니다(뱃지에 마우스를 올리면 실제 비교 시점이 나옵니다). 지금 걸어둔 검색·필터 <b>안에서</b> 다시 매긴 순위라, 예를 들어 <b>이탈리아</b>만 걸면 그 안에서의 등수 변화가 됩니다. 그때 표본 게이트를 못 채웠던 카드는 <b>NEW</b>입니다. <b>주의 — 큰 숫자가 곧 큰 실력 변화는 아닙니다.</b> 순위표는 가운데가 매우 빽빽해서, 중앙값(약 50%) 부근은 <b>±0.5%p 안에만 200장 넘게</b> 몰려 있습니다. 선방률이 조금만 움직여도 순위는 수백 위가 밀립니다(실제로 변동이 큰 카드는 표본이 적은 쪽이 아니라 오히려 <b>많은</b> 쪽입니다). 순위 등락보다 <b>선방률 값과 그 옆의 ±%p</b>를 함께 보세요. 다른 지표·정렬로 보는 중이면 왼쪽 #열과 뜻이 달라져 흐리게 표시됩니다.</li>
...
function trendChartHtml(c){
  ...
  // viewBox 를 컨테이너 종횡비에 가깝게 잡는다. 560x150 으로 두면 넓은 화면에서
  // preserveAspectRatio 가 높이(max-height)에 맞춰 축소해 좌우 170px 씩이 빈다.
  const narrow=window.innerWidth<640;
  const W=narrow?520:1000, H=narrow?230:250;
  const PL=narrow?46:62, PR=narrow?14:20, PT=narrow?16:20, PB=narrow?30:36;
  ...
  const lines=segs.filter(g=>g.length>1).map(g=>
    ...
    `fill="none" stroke="#d9b45a" stroke-width="3" stroke-linejoin="round"/>`).join('');
  const lastI=TP.length-1;
  // 점(7px)은 마우스로 맞히기엔 너무 작고, 브라우저 기본 <title> 툴팁은 1초쯤
  // 머물러야 뜬다. 투명한 큰 히트원을 겹치고 CSS :hover 로 값을 즉시 띄운다.
  const dots=series.map((s,i)=>{
    ...
    return `<g class="tpt" tabindex="0" aria-label="${TP[i]} ${s.rank}위, 전체 ${s.pool}장, 선방률 ${pct(s.pct)}">`+
      ...
      `fill="${i===lastI?'#e2707a':'#d9b45a'}" pointer-events="none"/>`+
      `<text class="ttip" x="${x.toFixed(1)}" y="${(above?y-24:y+32).toFixed(1)}" `+
      `text-anchor="${anchor}">${TP[i]} · ${s.rank}위 / ${s.pool}장 · ${pct(s.pct)}</text></g>`;
  }).join('');
  ...
  return `<h4 style="margin-top:14px">선방률 순위 추이 `+
    ...
    `<text x="6" y="${PT+5}" class="tlab">${yTop}위</text>`+
    `<text x="6" y="${H-PB}" class="tlab">${yBot}위</text>`+
    lines+dots+
...
function setGate(value){
  ...
  minGate=Math.min(50000,Math.max(100,Number.isFinite(n)?n:500));
